...
import { SignupForm } from '@zoom-out/ui'
...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { data, error: apiError } = await api.auth.signup.post({
        email,
        password,
        displayName,
      })

      if (apiError) throw new Error(apiError.value?.message || 'Error al crear cuenta')
  ...
  return (
    <div className="grid min-h-svh lg:grid-cols-2 bg-background font-sans overflow-hidden">
      <div className="relative hidden bg-muted lg:block overflow-hidden border-r border-border/50 animate-in fade-in slide-in-from-left-12 duration-1000">
        <div className="absolute inset-0 bg-linear-to-b from-background via-transparent to-background/20 z-10" />
        ...
        <div className="absolute bottom-12 left-12 right-12 z-20 space-y-6">
          ...
          <blockquote className="space-y-4">
            <p className="text-2xl font-black text-primary leading-tight tracking-tight uppercase">
              Expande tu <br/>
              Alcance <span className="text-muted-foreground/40 font-light text-lg">v4.0</span>
            </p>
            <p className="text-sm text-muted-foreground/60 leading-relaxed max-w-md uppercase tracking-widest font-bold">
              Únete a la red descentralizada de comunicaciones de próxima generación.
            </p>
          </blockquote>
        </div>
      </div>
